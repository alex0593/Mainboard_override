...
object TutorialCatalog {
    ...
    val lessons: List<TutorialLesson> = listOf(
        ...
        TutorialLesson(base().copy(board = base().board.copy(
            ...
            step(16, TutorialInput.SelectTile("a"), pauseBeforeAction = true), step(17, TutorialInput.Cell(Position(0, 2))),
            step(18, TutorialInput.SelectScript("KILL_PROCESS"), pauseBeforeAction = true),
            step(19, TutorialInput.Cell(Position(4, 3))),
            step(20, TutorialInput.EndTurn))),
        TutorialLesson(base().copy(dominoHand = listOf(Domino("a", 1, 2))), listOf(
            step(21, TutorialInput.SelectScript("SPOOF"), pauseBeforeAction = true), step(22, TutorialInput.SelectTile("a")),
            step(23, TutorialInput.Half(1)), step(24, TutorialInput.Value(3)), step(25, TutorialInput.Cancel),
            step(26, TutorialInput.SelectScript("SPOOF")), step(27, TutorialInput.SelectTile("a")),
            step(28, TutorialInput.Half(0)), step(29, TutorialInput.Value(0)),
            step(30, TutorialInput.ApplySpoof), step(31, TutorialInput.SelectTile("a")),
            step(32, TutorialInput.Cell(Position(0, 2))))),
        TutorialLesson(base().copy(board = base().board.copy(firewalls = setOf(Position(0, 2)))), listOf(
            step(33, TutorialInput.SelectScript("KILL_PROCESS"), pauseBeforeAction = true), step(34, TutorialInput.Cell(Position(0, 2))),
            step(35, TutorialInput.SelectTile("a")), step(36, TutorialInput.Cell(Position(0, 2))))),
        TutorialLesson(base().copy(board = base().board.copy(placed = listOf(first), firewalls = setOf(Position(2, 2))),
            ...
            step(37, TutorialInput.SelectScript("BRIDGE"), pauseBeforeAction = true), step(38, TutorialInput.ToggleBridge),
            step(39, TutorialInput.Cell(Position(2, 2))), step(40, TutorialInput.SelectTile("b")),
            step(41, TutorialInput.Cell(Position(3, 2)))), initialHorizontalBridge = false),
        TutorialLesson(base().copy(board = base().board.copy(width = 6, extraction = Position(6, 2),
            placed = listOf(first,
            PlacedDomino(Domino("b", 2, 4), Position(2, 2), Orientation.HORIZONTAL))),
            dominoHand = listOf(Domino("c", 4, 6))), listOf(
            step(42, TutorialInput.SelectTile("c"), pauseBeforeAction = true), step(43, TutorialInput.Cell(Position(4, 2))),
            step(44, TutorialInput.EndTurn), step(45, TutorialInput.Next), step(46, TutorialInput.Next))),
    )
